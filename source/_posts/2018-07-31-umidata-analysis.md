...
![](https://raw.githubusercontent.com/ProfessionalFarmer/f4w/master/2020/2018-07-30-UMI-analysis.png)
...
大致流程为：
<pre>-----prepare analysis ready BAM file------
'         FASTQ
'            ↓
'          uBAM
'            ↓  Extract UMI
...
<pre>java -Xmx8G -jar picard.jar FastqToSam \
    FASTQ=$fq1    \
    FASTQ2=$fq2   \
    OUTPUT=test.uBAM \
    ...
    RUN_DATE=`date --iso-8601=seconds`</pre>

## 2）提取UMI信息

<pre>java -jar fgbio.jar ExtractUmisFromBAM \
    --input=test.uBAM  \
    --output=test.umi.uBAM \
    --read-structure=2M148T 2M148T \
    --single-tag=RX \
    --molecular-index-tags=ZA ZB</pre>
	
此时，uBAM文件中RX标签记录着UMI的信息，2M148T表示前两个碱基是UMI分子标签，148个template 测序序列，配对read在uBAM文件中有如下信息

<pre>R1----ZA:Z:TC ZB:Z:TA RG:Z:test       RX:Z:TC-TA
R2----ZA:Z:TC ZB:Z:TA RG:Z:test       RX:Z:TC-TA</pre>

<!--more-->

## 3）比对uBAM文件中的reads

<pre>samtools fastq  test.umi.uBAM ' bwa mem -t 8 -p reference.fa /dev/stdin ' samtools view -b
 > test.umi.BAM</pre>

## 4）uBAM和BAM合并

<pre>java -Xmx8G -jar picard.jar MergeBAMAlignment R=reference.fa \
...
<pre>java -jar fgbio.jar GroupReadsByUmi \
    --input=test.umi.merged.BAM \
    --output=test.umi.group.BAM  \
    --strategy=paired  --min-map-q=20  --edits=1 --raw-tag=RX</pre>
...
<pre>java -jar fgbio.jar  CallMolecularConsensusReads \
    --min-reads=1 \
    --min-input-base-quality=20 \
    --input=test.umi.group.BAM \
    --output=test.consensus.uBAM</pre>

## 3）比对uBAM文件中的reads

<pre>samtools fastq $dir/$smp.callconsensus.BAM ' bwa mem -t 8 -p reference.fa  /dev/stdin ' samtools view -b - ' test.consensus.BAM</pre>
...
<pre>java -jar fgbio.jar FilterConsensusReads \
...
VarDict/bin/VarDict -G reference.fa -f $AF_THR -N test \
    ...
    VarDictJava/VarDict/var2vcf_valid.pl -N test -E -f $AF_THR > test.vcf</pre>
